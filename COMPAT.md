...
| `VAL-VECTOR` | Partial | Public vectors remain arrays. A declared `array<float, N>` normalizes integers to finite floats and enforces exactly `N` elements; bound query arrays may reach the 65,536 dimension ceiling. | `P9-PARSE-001`, `P9-PARSE-004` | [Phase 9 vector observations](docs/compat-research/phase9.md) | `P9-VECTOR-001`, `P9-VECTOR-002`, `P9-API-001`, `P9-CLI-001` |
...
| `OP-KNN` | Partial | Execute exactly one top-level-AND KNN predicate using the COSINE or EUCLIDEAN metric with `K` in `1..=10,000`; proven scalar filters run before exact top-k. OR/NOT, dynamic K, multiple KNN predicates, and ANN methods are rejected. | `P9-PARSE-002`, `P9-PARSE-004` | [Phase 9 vector observations](docs/compat-research/phase9.md) | `P9-VECTOR-001`, `P9-VECTOR-002`, `P9-VECTOR-003` |
| `EXPR-VECTOR-FN` | Partial | Execute `vector::distance::euclidean`, `vector::similarity::cosine`, and KNN-associated `vector::distance::knn()` projections. Other vector function paths fail explicitly. | `P9-PARSE-003`, `P9-PARSE-004` | [Phase 9 vector observations](docs/compat-research/phase9.md) | `P9-VECTOR-001`, `P9-API-001` |
...
| `EXPR-EXCLUDED` | Unsupported | Function calls outside the closed Phase 8 FTS and Phase 9 vector sets, casts, subqueries, ranges, indexing, modulo, power, and broader operators fail explicitly. | `P1-EXPR-005`, `P6-AST-001`, `P8-PARSE-004`, `P9-PARSE-004` | [Phase 1 exclusions](docs/compat-research/phase1.md#binding-power-and-exclusions), [Phase 9 vector observations](docs/compat-research/phase9.md) | `P1-BRIDGE-003`, `P6-LANG-002`, `P8-FTS-009`, `P9-VECTOR-003` |
...
| `SCHEMA-FIELD` | Partial | Execute DEFINE FIELD on an existing table and validate all existing rows atomically, including fixed `array<float, N>` fields. Broader parameterized collection types remain rejected. | `P1-STMT-001`, `P9-PARSE-001` | [Phase 2 fields](docs/compat-research/phase2.md#required-optional-null-and-numeric-coercion), [Phase 9 vector observations](docs/compat-research/phase9.md) | `P2-SCHEMA-005`, `P3-ATOMIC-003`, `P9-VECTOR-005` |
...
| `TYPE-VECTOR` | Partial | Enforce fixed `array<float, N>` for `N` in `1..=65,536`, retaining arrays publicly and a sealed native `vector64` representation internally. Other typed-array element forms are rejected. | `P9-PARSE-001`, `P9-PARSE-004` | [Phase 9 vector observations](docs/compat-research/phase9.md) | `P9-VECTOR-001`, `P9-VECTOR-002`, `P9-VECTOR-005` |
...
| `EXCL-ADVANCED` | Unsupported | Namespaced functions and analyzers outside the closed Phase 8 FTS and Phase 9 vector subsets, casts, subqueries, permissions, users, namespaces, scopes, events, views, live queries, ANN/vector indexes, and other specialized indexes remain unavailable. | `P6-AST-001`, `P1-STMT-005`, `P8-PARSE-004`, `P9-PARSE-004` | [Phase 1 exclusions](docs/compat-research/phase1.md#phase-1-interpretation), [Phase 9 vector observations](docs/compat-research/phase9.md) | `P1-BRIDGE-003`, `P6-LANG-002`, `P8-FTS-009`, `P9-VECTOR-003` |
...
## Phase 9 exact-vector contracts

- Fixed vector fields retain normalized arrays in documents and atomically maintain one catalog-owned `vector64` BLOB. Catalog ownership, physical type, dimensions, encoding, and document/BLOB agreement validate on reopen.
- KNN is an exact native linear scan ordered by distance and encoded RID with physical `LIMIT K`. `EXPLAIN` labels the exact metric and K and never implies ANN.
- COSINE and EUCLIDEAN accept literal or bound finite vectors of the declared dimension. COSINE rejects a zero query vector. HNSW, DiskANN, and `toy_vector_sparse_ivf` remain explicitly unsupported.
