...
pub fn translate_drop_table(
    ...
    let indices: Vec<_> = resolver.with_schema(database_id, |s| {
        // Row maintenance hides backing B-trees through get_indices(), but
        // table teardown must also free the storage owned by index methods.
        s.indexes
            .get(normalize_ident(tbl_name.name.as_str()).as_str())
            .into_iter()
            .flatten()
            .cloned()
            .collect()
