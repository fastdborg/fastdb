...
impl Connection {
    ...
    /// Test-only: run `EXPLAIN QUERY PLAN` over the **actual translated
    /// FastDB filter statement**. The explained SQL is generated by rendering
    /// (`to_string()`) the same `turso_parser::ast::Stmt` that
    /// `physical_select_by_field_stmt` builds for the filter, so the plan
    /// tracks the real FastDB lowering — the test fails if the lowering
    /// changes the expression shape. (A direct `Cmd::ExplainQueryPlan` path
    /// over the translated AST would need a new public engine API; rendering
    /// the lowered AST is the equivalent without a core change.)
    #[cfg(feature = "testing")]
    #[doc(hidden)]
    pub fn explain_field_filter(&self, logical_table: &str, field: &str) -> Result<Vec<String>> {
        ...
        // Render the lowered AST and explain it; EXPLAIN QUERY PLAN does not
        // execute, so the `?1` parameter needs no value bound.
        let explain_sql = format!("EXPLAIN QUERY PLAN {select_stmt}");
