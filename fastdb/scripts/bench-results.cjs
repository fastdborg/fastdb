...
(async () => {
  if (process.argv[2] === '--child') {
    const [client,operation,countText,widthText] = process.argv.slice(3);
    ...
    try {
      ...
      global.gc();
      const before=memory();
      const sql='SELECT n,zeroblob('+width+') AS payload FROM positions ORDER BY n';
      ...
      console.log(JSON.stringify({client,operation,count,width,logicalCellBytes:count*(8+width),elapsedMs,before,after,
  ...
  for (const client of ['sync','worker']) for (const operation of ['execute','profile']) for (let sample=0;sample<3;sample++) {
    const output=execFileSync(process.execPath,['--expose-gc',__filename,'--child',client,operation,String(count),String(width)],{cwd:root,encoding:'utf8',timeout:120000,maxBuffer:1024*1024});
