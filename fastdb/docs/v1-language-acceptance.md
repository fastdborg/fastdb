...
| 2. Grammar and parameter ambiguities | parser/; tests/tests/standalone.rs; tests/tests/sql_compat.rs | Inspect individual grammar cases before closing |
...
| 9. One-hop links | tests/tests/links.rs | Named tests cover batching, typed identity, relational targets, transaction snapshots and shared byte budget; verify forbidden multi-hop rejection |
...
overflow rejection. Before closing item 2, inspect runtime binding assertions for
the five supported parameter forms; parser dispatch alone is insufficient.

Items 1 and 3 have existing assertions for their explicit checklist requirements;
no additional test matrix is requested for them. This evidence review does not
