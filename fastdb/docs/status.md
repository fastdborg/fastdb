...
- Dense vector32/vector64 values, vector<N> field validation, and initial exact-distance/extraction lowering use the pinned engine.
...
3. Complete forward-link resource/planner coverage and upstream vector representation/operation coverage; add bundled bounded QuickJS functions. Dense vector32/vector64 validation and initial exact-distance queries now exist; other representations and broader vector release evidence remain pending.
