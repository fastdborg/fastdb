...
Collection/native-column `<`, `<=`, `>` and `>=` now use raw binary payloads, preserve native numeric affinity and text collation, and reject non-null record/scalar ordering. Regression coverage includes both operand orders, native column wrappers, NULLs, left joins, typed derived/CTE sources and native INSERT SELECT with explicit rollback. Mixed BETWEEN, arbitrary native expressions and broader logical-operand wrappers remain to be qualified.
