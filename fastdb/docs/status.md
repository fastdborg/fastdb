...
An initial version of `binary_check_operators_validate_payloads_atomically_after_reopen` combined all four constraint groups into one left-associated AND expression and overflowed the default Rust test thread stack (Rust 1.88.0, unoptimized Linux build). The operator regression uses separate field constraints to isolate scalar semantics. This does not resolve the overflow: reconstruct the single constraint by joining the four CHECK strings in that test with AND against one `valid` field. Expression depth/stack accounting and a controlled failure instead of process abort remain release work.
