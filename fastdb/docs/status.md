...
A bounded subprocess harness now kills repeated mixed document/relational transactions near COMMIT and TRUNCATE checkpoint calls, using zero-, one- and five-millisecond delays after flushed phase markers. Each of six fresh databases is reopened twice. Checks cover native integrity, every acknowledged commit, at most one additional unacknowledged durable transaction, complete contiguous batches, typed binary payloads, unique index lookups and absence of the next batch's documents/index entries.
