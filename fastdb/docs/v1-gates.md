...
This is a navigation and prioritization aid, not a replacement for the parent FastDB.md and FastQL.md plans. The current implementation is not release-complete. Most recent complete scoped evidence: 636 passing Rust tests with one ignored trigger-cancellation gate, 89 passing Node/application tests, formatting, Clippy and strict TypeScript (`/tmp/fastdb-source-scope-check.log`, based on `e1d1c3748` plus isolated UPDATE FROM source validation). These totals do not prove the gates below. Installed-package evidence is separate: Linux Node 22/24 now cover leading RIGHT/LEFT UPDATE FROM, target-alias CTEs and pagination. The standalone Rust consumer also covers joined RIGHT/LEFT UPDATE FROM, target-alias CTEs, pagination and committed reopen. See verification.md for exact runs and limitations.
