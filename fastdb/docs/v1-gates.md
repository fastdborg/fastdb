...
This is a navigation and prioritization aid, not a replacement for the parent FastDB.md and FastQL.md plans. The current implementation is not release-complete. Most recent scoped evidence: 426 passing Rust tests with one ignored trigger-cancellation gate, formatting and Clippy; after a Node test-fixture correction, 48 passing Node/application tests and strict TypeScript. Later focused checks are recorded below and in verification.md. Installed-package evidence is recorded separately. See verification.md for exact runs and limitations.
