...
The [V1 release preview checklist](preview-release.md) now governs delivery priority. This document is the stable-V1 backlog and historical evidence, not an open-ended preview checklist.

This is a navigation and prioritization aid, not a replacement for the parent FastDB.md and FastQL.md plans. The current implementation is not release-complete. Most recent complete scoped evidence: 644 passing Rust tests with one ignored trigger-cancellation gate, 89 passing Node/application tests, formatting, Clippy and strict TypeScript (`/tmp/fastdb-update-rollback-check.log`, based on `49253c971` plus UPDATE OR ROLLBACK support). These totals do not prove the gates below. Installed-package evidence is separate: Linux Node 22/24 now cover leading RIGHT/LEFT UPDATE FROM, target-alias CTEs and pagination. The standalone Rust consumer also covers joined RIGHT/LEFT UPDATE FROM, target-alias CTEs, pagination and committed reopen. See verification.md for exact runs and limitations.
...
| Inner WITH scalar below | `(1,NULL),(4,3)` | Same after local-WITH scope fix | Broaden local-WITH parameters, nested scope, callback and write-recovery qualification |
...
Reference probes (all three now have implementation evidence; broader scope qualification remains open):
