...
This is a navigation and prioritization aid, not a replacement for the parent FastDB.md and FastQL.md plans. The current implementation is not release-complete. Most recent complete scoped evidence: 562 passing Rust tests with one ignored trigger-cancellation gate, 80 passing Node/application tests, formatting, Clippy and strict TypeScript (`/tmp/fastdb-iterator-names-check.log`, implementation tree based on `b7fc98e7c` with unqualified iterator argument resolution). Later focused runs are recorded in status.md with their exact scope. Installed-package evidence is recorded separately. See verification.md for exact runs and limitations.
