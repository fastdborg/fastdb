...
OR FAIL restores the failing candidate and stops on mutation validation or
constraint failure, retaining earlier successful candidates before returning the
error. In autocommit those earlier changes commit; in an explicit transaction
they remain pending. No partial RETURNING result is delivered on error. Other
...
clause and native JSON iterator arguments, but is not an outer source for FROM
join constraints or derived source SELECTs. CTEs named like the
...
## Bundled string functions

The 2.1 candidate embeds fixed JavaScript source through pinned `rquickjs = 0.13.0` (MSRV 1.87), bundling QuickJS-NG 0.16.2. See [the dependency review](dependency-security.md) for the runtime update. The public catalog currently contains:
...
Each call creates a fresh runtime with an 8 MiB JavaScript heap limit, 256 KiB stack limit, 65,536-byte UTF-8 input/output limits, and an interrupt handler that stops after 1,000 allowed polls or a 100 ms elapsed deadline. Interrupt polling is cooperative, not a hard real-time deadline. Output size is checked after conversion to a Rust string; these limits do not bound total query memory or total execution time across many rows. Settings use the [rquickjs Runtime API](https://docs.rs/rquickjs/0.13.0/rquickjs/struct.Runtime.html); allocator features that bypass its heap limit are not enabled.

These bundled helpers evaluate only compiled-in source. Inputs pass as function arguments, never interpolated code. No module loader, host I/O or database callbacks are installed. Separately, V2 provides the documented [sandboxed stored-function API](v2-user-functions.md); its limits and value contract are distinct from this fixed helper catalog.

Direct object-expression argument errors use validation errors; input/output, execution and recognized allocation/stack failures use `FDB_LIMIT`. Runtime initialization or unexpected bundle failures use storage errors. SQL UDF failures pass through the engine error path. The maintained scalar-read-error correction preserves caller transactions and named savepoints for read-only extension errors; write failures retain the documented rollback/conflict rules. Inspect transaction reports after errors.

Fresh runtimes trade performance for isolated calls. The Linux release includes cancellation, native packaging and attribution qualification; the [2.1 checklist](v2.1-tasks.md) tracks updated runtime, artifact and workload evidence. Non-Linux platforms are outside the current release scope. Runtime/Unicode upgrades can affect deterministic outputs and must be tested against application expectations.
...
Expanded columns participate in GROUP BY ordinals and INSERT SELECT width/validation checks. Public result queries and positional INSERT SELECT accept duplicate projection names, preserving names and values by column position. Name-based ORDER BY resolves the first matching projection; ordinal ordering selects its specified position. Initial USING joins now resolve merged keys and suppress joined key columns from unqualified closed-source stars; INNER/LEFT/leading-RIGHT and chained keys have initial coverage. NATURAL and broader table-function/join-scope qualification remain unresolved. Full concurrent schema-change and view-dependency authorization qualification is still pending.
...
Native derived stars with duplicate output names preserve column positions in supported INSERT SELECT queries. Later duplicate columns receive private runtime names while ordinary name lookup retains the first matching column. Case-insensitive duplicates, distinct scalar/binary/NULL values and rollback are covered. Public queries also preserve these positional results. Logical derived sources also preserve duplicate names and per-position types through nesting and supported writes. Qualified name lookup resolves the first case-insensitive match. Collection CTEs also preserve duplicate inferred or explicitly listed output names through chained stars and supported writes, including MATERIALIZED definitions. Fetched derived/CTE projections retain their restrictions; mixed native CTE references preserve duplicate star positions through shared companion CTEs, including chained, MATERIALIZED and NOT MATERIALIZED definitions. Callback regressions compare one/two references and LIMIT 0 with the pinned engine and verify that EXPLAIN does not execute the source. Direct qualified native columns use the source metadata spelling for default labels, including first-match duplicate CTE names. Broader collation/evaluation and expression-label formatting compatibility remain open.
