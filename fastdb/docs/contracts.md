...
Native execution and profiling now return their existing parser/validation errors for the tested oversized expressions; collection execution retains its existing managed-lowering fallback error. CLI, two-MiB Rust caller-thread and synchronous/worker Node regressions verify process survival, retained active work and subsequent queries. A schema-change test verifies actual engine reprepare of accepted deep expressions. This is not a whole-process memory cap, an assurance for arbitrary caller stack sizes or a cross-platform stack qualification; frontend recursive lowering and other execution paths still require broader limits.
