...
Differential collection/native-BLOB tests cover ascending/descending ordering, DISTINCT, range predicates, literal projections, indexed IN ordering, and DELETE/RETURNING rollback. This does not add managed range-index seeks. Arbitrary expression type propagation, mixed-type ordering, native-function left operands compared with typed list entries, and broader CHECK comparison contexts still need qualification. Existing prototype CHECK definitions affected by typed binary comparison corrections should be reapplied to validate stored documents; opening alone does not revalidate them.
