...
The compound remains one engine query. Generated arm definitions and the compound definition share the user WITH scope; MATERIALIZED user definitions remain shared across arm references. CTE/derived consumers retain typed output columns. ORDER BY accepts first-arm output names or positions, retaining explicit collation, direction and NULL placement; LIMIT/OFFSET apply to the combined rows. Profiling and both EXPLAIN forms use the compound plan. Ordering by arbitrary expressions or aliases introduced only in later arms remains unsupported on this path.
