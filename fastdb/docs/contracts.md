...
The initial Rust error categories have stable identifiers exposed by code(). Engine errors retain their native message. Busy and stale-snapshot contention use FDB_BUSY and FDB_BUSY_SNAPSHOT, and interruption uses FDB_CANCELLED; other engine failures still use FDB_ENGINE, with constraint and other subcategories pending. SELECT/RETURNING produce rowsets with all/first/exactly_one helpers; a collection document occupies one Object-valued column named document. Commands without RETURNING have no rows. Ordinary SQL columns retain engine names, types, and positional ordering. Final duplicate-column and client/wire contracts remain release gates.
