...
Tests cover indexed and unindexed results, duplicate/list/null behavior, reversed equality, typed parameters and DELETE/RETURNING rollback. Literal projections and ordinary relational comparisons retain native behavior. Broader binary expression propagation and range ordering remain unfinished. Native function/cast inputs are covered by the extension below; CHECK and other expression contexts still need qualification. No persisted format changed.
...
Differential tests compare length, hex, typeof, substr, nested/coalesce inputs and text/integer CAST with native BLOB columns, including empty and null values. They also cover a typed parameter, indexed equality, UPDATE/RETURNING with binary function results and rollback/index maintenance. This fixes the earlier length(data) probe reporting encoded length rather than payload length. Broader binary expression/range propagation, CHECK arguments, aggregate/window qualification and total resource budgets remain release work; no persisted encoding changes.
