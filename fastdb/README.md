...
Use a fresh database for the example. `fastdb/examples/sql-writes.fastql` additionally exercises column-list inserts, nested SET, and transaction rollback. The prototype CLI reads semicolon-delimited scripts from stdin, supports multiline statements, and emits tagged JSON results with statement byte offsets. It stops at the first execution error and exits nonzero. Use `--line` for legacy one-statement-per-line processing that continues after errors; this mode also exits nonzero if any statement fails. It is not the final import/export or client wire format.
