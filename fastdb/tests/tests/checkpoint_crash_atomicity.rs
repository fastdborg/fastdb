...
fn query_rows(conn: &Arc<turso_core::Connection>, sql: &str) -> TestResult<Vec<String>> {
...
fn run_checkpoint_crash_scenario(sync_pragma: &str) -> TestResult<()> {
    ...
    if sync_pragma == "FULL" {
        assert_eq!(
            distinct,
            vec![new.clone()],
            "FULL must recover the acknowledged transaction T, not its predecessor"
        );
    }
    assert!(
...
/// A crash during checkpoint backfill under `synchronous=NORMAL` must recover
/// to a committed prefix: the database equals
...
/// must be clean.
#[test]
fn checkpoint_backfill_crash_under_synchronous_normal_recovers_committed_prefix() -> TestResult<()>
{
...
#[test]
fn checkpoint_backfill_crash_under_synchronous_full_control() -> TestResult<()> {
