...
fn collection_windowed_scalar_pagination_matches_native() {
    ...
    for offset in [0, 1, 2, 3] {
        let native = format!("SELECT n,(SELECT row_number() OVER (ORDER BY i.n) FROM lookup i WHERE i.n>=d.n ORDER BY i.n LIMIT 1 OFFSET {offset}) FROM baseline d ORDER BY n");
        let expected = q(&c, &native).rows;
        for source in ["docs d", "(SELECT n FROM docs) d"] {
            let sql = format!("SELECT n,(SELECT row_number() OVER (ORDER BY i.n) FROM items i WHERE i.n>=d.n ORDER BY i.n LIMIT 1 OFFSET {offset}+0) FROM {source} ORDER BY n");
            assert_eq!(q(&c, &sql).rows, expected, "{sql}");
            assert_eq!(
                c.profile_select(&sql, &Parameters::new())
                    .unwrap()
