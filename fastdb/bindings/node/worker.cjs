...
parentPort.on('message', ({ id, method, args, cancellationKey }) => {
  try {
    ...
    const result = method === 'execute' && cancellationKey !== undefined
      ...
      : (method === 'profileSelect' || method === 'checkCollectionIntegrity' || method === 'executeBatch' || method === 'importDocuments' || method === 'exportDocuments' || method === 'migrate') && cancellationKey !== undefined
