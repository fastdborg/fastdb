...
const methods = new Set(['execute', 'profileSelect', 'profileSelectWithLimits', 'checkCollectionIntegrity', 'executeBatch', 'migrate', 'exportDocuments', 'importDocuments', 'close']);
parentPort.on('message', ({ id, method, args, cancellationKey }) => {
  try {
    ...
    const result = method === 'execute' && cancellationKey !== undefined
      ...
      : (method === 'profileSelect' || method === 'profileSelectWithLimits' || method === 'checkCollectionIntegrity' || method === 'executeBatch' || method === 'importDocuments' || method === 'exportDocuments' || method === 'migrate') && cancellationKey !== undefined
