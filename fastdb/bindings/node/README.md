...
handlers, prefer [AsyncDatabase](#asynchronous-database) to keep database work off
the main JavaScript thread. The synchronous Database API remains useful for
scripts and other blocking workflows. Browser/WASM support is not part of V2.

From the repository root, install the declaration checker and run the scoped checks:

```sh
pnpm --dir fastdb/bindings/node install --ignore-scripts
...
For an optimized local addon, run `fastdb/scripts/check-node.sh --release` before the package smoke. The script uses the pinned workspace release profile and runs the same client/application tests and TypeScript check as the default debug build. It copies the selected artifact into the private package; the subsequent package smoke validates that actual artifact. This option does not build universal prebuilds or publish a package. On Linux, `--release-stripped` additionally runs `strip --strip-debug` on the package copy before the same checks, retaining the full release artifact under Cargo’s target directory for diagnostics. It requires `strip`; other platforms reject this option.
...
Importing on Node versions below the declared minimum of 22 throws `FDB_RUNTIME_VERSION` before loading native code.
...
**It does not bound engine RETURNING buffers, write candidates, document
snapshots or individual projection working memory.** Transaction control, DDL, reads and multiple
